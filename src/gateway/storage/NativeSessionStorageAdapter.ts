...
/** Catalog view over the same host-owned native storage used by sessions. */
export function createGatewayNativeSessionCatalog(
  adapter: GatewayNativeSessionStorageAdapter,
): SessionCatalogPort {
  return Object.freeze({
    async list(input) {
      if (adapter.listSessions) {
        return adapter.listSessions(Object.freeze({
          projectRoot: input.projectRoot,
          pilotHome: input.pilotHome,
          ...(input.limit !== undefined ? { limit: input.limit } : {}),
          ...(input.offset !== undefined ? { offset: input.offset } : {}),
          ...(input.includeInternal !== undefined ? { includeInternal: input.includeInternal } : {}),
        }));
      }
      return listProjectSessions({
        ...input,
        chatDir: resolveGatewayNativeProjectChatDir(input, adapter),
      });
    },
  });
}
