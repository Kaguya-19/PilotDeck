...
import { InMemorySessionProjectionCheckpointStore } from "../../session/projection/checkpoint/index.js";
...
export function createGatewayAsyncTranscriptStorageAdapter(
  ...
  const createStorage = (input: GatewayNativeSessionStorageInput): AgentProjectSessionStorage => {
    ...
    const storageProvider: ProjectSessionStorageProvider = {
      create(storageInput) {
        const key = keyForPath(storageInput.transcriptPath);
        return {
          persistence: {
            append: async (entry) => {
              await options.store.append(key, structuredClone(entry));
            },
            load: async () => cloneReadResult(await options.store.read(key)),
            flush: async () => {},
          },
    ...
    const base = options.createSessionStorage?.(input) ?? createAgentProjectSessionStorage({
      ...input,
      storageProvider,
    });
