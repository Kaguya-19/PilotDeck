...
1. [Module Communication SOP](pilotdeck-module-communication-sop.zh.md)

   规范身份字段、operation/attempt 状态、终态、取消、deadline、重试、恢复、profile
   和 transport-independent adapter 约定。当前文档版本为 v0.3，协议版本为 v2.0。

2. [Module Protocol v2 Schema](pilotdeck-module-protocol-v2.schema.json)

   机器可读的 request、response、event、error、module_call 和 host module 字段定义。

3. [AgentLoop Modular Framework TRD](trd/03-agent-loop-modular.zh.md)

   说明 `ModelInvokerPort`、`ToolPort`、`AgentContextRuntime`、sidecar factory、
   context module 和 capability module 的实现边界。

4. [StaffDeck AgentLoop Integration](https://github.com/Kaguya-19/StaffDeck/tree/codex/pilotdeck-agent-loop/docs/pilotdeck-agent-loop-integration.md)
...
5. [AgentLoop parity README](https://github.com/Kaguya-19/StaffDeck/tree/codex/pilotdeck-agent-loop/tools/agent-loop-parity/README.md)

   对拍 harness、mock provider/tool、canonical trace 和真实部署验证方法。对拍脚本属于
   StaffDeck 测试设施，不属于 PilotDeck AgentLoop runtime。
