...
export function useChatModelSelection({ projectKey = '', sessionId = '' }: { projectKey?: string; sessionId?: string } = {}) {
  ...
  const [history, setHistory] = useState<{ key: string; selection: ChatModelSelection | null; error: string | null } | null>(null);
  const local = sessionId ? readSessionModelSelection(projectKey, sessionId) : null;
  useEffect(() => { void globalModelSelectionStore.load(); }, []);
  useEffect(() => {
    setDraft(null);
    setHistory(null);
    if (!projectKey || !sessionId || readSessionModelSelection(projectKey, sessionId)) return;
    ...
    void (async () => {
      try {
        ...
        if (!controller.signal.aborted) setHistory({ key, selection: normalizeModelSelection(data.saved), error: null });
      } catch (error) {
        if (!controller.signal.aborted) setHistory({ key, selection: null, error: error instanceof Error ? error.message : String(error) });
      }
    })();
  ...
  const restoring = Boolean(projectKey && sessionId && !local && !currentHistory && !currentDraft);
  const selection = currentDraft || local || currentHistory?.selection || state.selection;
  const error = modelSelectionError({ ...state, selection, error: state.error || (!local && !currentDraft ? currentHistory?.error : null) || null });
